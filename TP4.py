...
def farm_factory(farm_name, animal_description):
    ...
    for animal in animal_description:
        datetime_animal = datetime.datetime.strptime(animal['birthdate'], '%d %B %Y')
        my_current_farm.add_animal(animal['type'], animal['sex'] ,datetime_animal)
...
if __name__ == "__main__":
	...
	with open('farmjson.json') as json_file:
	...
	for farm in data:
		farm_list.append(farm_factory(farm['Farm_name'],farm['Animal list']))
